...
def predict():
    ...
    # Estimate price per GB and MP for feature engineering
    # We'll use average values from the dataset for these calculations
    avg_price_per_gb = df['Price'].mean() / df['Internal storage (GB)'].mean()
    avg_price_per_mp = df['Price'].mean() / (df['Rear camera'].mean() + df['Front camera'].mean())
    
    price_per_gb = avg_price_per_gb
    price_per_mp = avg_price_per_mp
